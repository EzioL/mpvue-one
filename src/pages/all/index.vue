...
<script>
  ...
  export default {
    ...
    data() {
      return {
        banner: [], topics: [], items: [], isRefresh: true, isLoading: false, isLoadMore: false
      }
    }, created() {
      ...
      requestTopicData: function () {
        ...
        await Promise.all([this.requestBannerData(), this.requestTopicData()])
      }, routerTopicList(topic) {
        console.log("routerTopicList", topic)
        const targetUrl = "/pages/topic/list/main" + "?content_id=" + topic.content_id + "&cover=" + topic.cover;
